...
Case Case_class::copy_Case()
{
   return new Case_class(copy_Symbol(name), copy_Symbol(type_decl), expr->copy_Expression());
}

void Case_class::dump(std::ostream &stream, int n)
{
   stream << pad(n) << "branch\n";
   dump_Symbol(stream, n + 2, name);
   dump_Symbol(stream, n + 2, type_decl);
...
{
   return new Case_class(name, type_decl, expr);
}
...
//
// Case_class::dump_with_types dumps the name, type declaration,
// and body of any case branch.
//
void Case_class::dump_with_types(std::ostream &stream, int n)
{
   dump_line(stream, n, this);
